...
> using System.Collections.Generic;
>
> List<string> list = new List<string> { "Gem_R_Wound", "Gem_E_Aftershock", "Gem_E_Might", "Gem_L_HeartOfGold", "Gem_L_Infinity", "Gem_L_SoulPrison", "Gem_L_SuppressedArcanum", "Gem_R_Adventure", "Gem_R_Blood", "Gem_R_Rejuvenation" };
>       Dew.CreateGem<Gem>(DewResources.GetByShortTypeName<Gem>(list[Star_Global_StartSkill.random.Next(list.Count)]), vector, 80, base.player, null);
...
```c#
//在不讨论保命的纯输出情况下，燃烧伤害是最暴力的
Star_Global_FireAmp
	——》
	 FireAmp = new float[] { 0.88f, 0.88f, 0.88f, 0.88f, 0.88f };
```
...
Star_Global_FirstGemUpgrade
	——》百分比
	BonusQuality = new int[] { 999, 999, 999};
```
...
Star_Global_StartGold
	——》
	GoldAmount = new int[] { 999, 999, 999 };
```
...
Star_Global_MaxHealth
	——》 默认最高是150f
	BonusHealth = new float[] { 300f, 300f, 300f, 300f };
```
...
——》
	...
	{
		num = Mathf.Min(num, victim.Status.maxHealth / 10f);
	}
```
